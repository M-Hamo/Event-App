...
import { IEvent, ISession } from '../events-list/shared/event.model';
...
export class EventService {
  ...
  saveEvent(event) {
    // this line return the gratest id in Events id
    const theGradeId = Math.max.apply(null, Events.map(event => event.id));
    event.id = theGradeId + 1;
    event.session = [];
    Events.push(event);
  }

  updateEvent(event) {
    // this line will find the event we are make a new session with it
  ...
  searchSession(searchTerm: string) {
    ...
    Events.forEach(event => {
      let matchingSessions = event.sessions.filter(session =>
        ...
        //session.name.toLocaleLoweCase().indexOf(term) > -1   //=> can be used too to find the term // indexOf() used to search in strings
      );
      // this line to add eventId to the session are belongs to
...
const Events: IEvent[] = [
  ...
  },
  {
    ...
    onlineUrl: '/mohamed.eldeib5@gmail.com',
    sessions: [
      {
        id: 1,
